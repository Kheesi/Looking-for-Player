    ...
    <div id="menu">
        <div id="formulaire">
            <form action="Joueurs.php" class="form-philtre" method="post">
                ...
                <p class="text">Rôles joués :</p>
                    <p>
                        <input type="checkbox" id="role1" name="role[]" value="Support">
                            <label for="role1">Support</label>
                    </p>
                    <p> 
                        <input type="checkbox" id="role2" name="role[]" value="Mid">
                            <label for="role2">Mid</label>
                    </p>
                    <p>  
                        <input type="checkbox" id="role3" name="role[]" value="Ad carry">
                            <label for="role3">Ad carry</label>
                    </p>
                    <p>  
                        <input type="checkbox" id="role4" name="role[]" value="Top">
                            <label for="role4">Top</label>
                    </p>
                    <p>  
                        <input type="checkbox" id="role5" name="role[]" value="Jungler">
                            <label for="role5">Jungler</label>
                    </p>
        ...
        <div id="affichage">
            <?php   
                ...
                if(!isset($_POST['filtre']))
                    {
                        $requete="SELECT * FROM Annonce Where type_annonce='0'";
                        $res=mysqli_query($connexion,$requete);
                        while($P=mysqli_fetch_array($res))
                            {
                                echo "<a href='Annonce.php?id_annonce={$P['id_annonce']}&'><h2 class='title'>{$P['titre']}</h2></a><br> Pseudo : {$P['pseudo']} <br> Ligue : {$P['ligue']} <br>";
                            }
                    }
                else
                    {
                                ...
                                if($serv==0)
                                    {
                                        if($ligue==0)
                                            ...
                                            {
                                                $requete="SELECT * FROM Annonce WHERE  ligue='$ligue' AND type_annonce='0'";
                                            }
                                    }
                                else
                                    {
                                        $requete="SELECT * FROM Annonce WHERE  serveur='$serv' AND ligue='$ligue' AND type_annonce='0'";}
                                        $res=mysqli_query($connexion,$requete);
                                        while($P=mysqli_fetch_array($res))
                                            {          
                                                echo "<a href='Annonce.php?id_annonce={$P['id_annonce']}'><h2 class='title'>{$P['titre']}</h2></a><br> Pseudo : {$P['pseudo']} <br> Ligue : {$P['ligue']} <br>";
